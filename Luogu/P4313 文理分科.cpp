...
signed main(){
	...
	rep(i,1,n)rep(j,1,m){
		...
		adde(tot,id(i,j),inf);
		if(i>1)adde(tot,id(i-1,j),inf);
		if(i<n)adde(tot,id(i+1,j),inf);
		if(j>1)adde(tot,id(i,j-1),inf);
		if(j<m)adde(tot,id(i,j+1),inf);
	}
	rep(i,1,n)rep(j,1,m){
		int x=read();ans+=x,adde(++tot,T,x);
		adde(id(i,j),tot,inf);
		if(i>1)adde(id(i-1,j),tot,inf);
		if(i<n)adde(id(i+1,j),tot,inf);
		if(j>1)adde(id(i,j-1),tot,inf);
		if(j<m)adde(id(i,j+1),tot,inf);
	}
