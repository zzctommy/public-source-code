...
void poly_inv(int*g,int*f,int n){//f[0,n)的逆元存在g[0,n),先清空g
	static int A[M];
...
void poly_sqrt(int*g,int*f,int pw){//f[0,n)的根号在g[0,n)，需要保证f[0]=1，先清空g
	static int A[M];
...
void poly_ln(int*g,int*f,int n){//f[0,n)的ln在g[0,n)，先清空g
	static int A[M],B[M];
	dao(A,f,n),clr(B,n),poly_inv(B,f,n),poly_mul(A,B,A,n,n),jif(g,A,n);
}
void poly_exp(int*g,int*f,int n){//f[0,n)的exp在g[0,n)，先清空g
	static int A[M];
...
void poly_qpow(int*g,int *f,int k,int n){//f[0,n)的k次幂在g[0,n)，先清空g
	static int A[M];
...
int n,k,f[M],g[M];
signed main(){
	n=read(),k=exread(),math::init_math(n);
	for(int i=0;i<n;++i)f[i]=read();
	poly::poly_qpow(g,f,k,n);
